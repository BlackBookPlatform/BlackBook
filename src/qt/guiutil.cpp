...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Blackbook address (e.g. %1)").arg("Bt5kR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Blackbook: URI
...
{
    // Convert blackbook:// to blackbook:
    //
    //    Cannot handle this later, because blackbook:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open blackbook.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Blackbook.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Blackbook.lnk
...
{
    return GetAutostartDir() / "blackbook.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a blackbook.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Blackbook\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the blackbook app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add blackbook app to startup item list
